...
export function deleteProjectMemories(db: Database, projectId: string): number {
  // Count first because bun:sqlite's changes includes FTS trigger side effects
  const count = (db.query(`SELECT COUNT(*) as count FROM memories WHERE project_id = $projectId`).get({ $projectId: projectId }) as any).count;
  
  if (count > 0) {
    db.query(`DELETE FROM memories WHERE project_id = $projectId`).run({ $projectId: projectId });
  }
